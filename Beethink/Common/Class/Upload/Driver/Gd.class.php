...
class Gd{
    ...
    public function open($imgname){
        //检测图像文件
        if(!is_file($imgname)) Error::halt('','不存在的图像文件');
    ...
    public function water($source, $locate = Image::IMAGE_WATER_SOUTHEAST,$alpha=80){
        //资源检测
        if(empty($this->img)){
            Error::halt(123,'没有可以被添加水印的图像资源');
        }
        if(!is_file($source)){
            Error::halt(123,'水印图片不存在');
        }
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        ...
        //资源检测
        if(empty($this->img)){
            Error::halt(123,'没有可以被写入文字的图像资源');
        }
        if(!is_file($font)){
            Error::halt(124,'不存在的字体文件');
        }
    ...
    public function rotate($angle,$color,$transparent=0)
    {
        ...
        do{
            $this->img=imagerotate($this->img,$angle,$col,$transparent);
        } while(!empty($this->gif) && $this->gifNext());
    }
    /*颜色值转换*/
