...
//    定义一些常用的过滤规则
     ...
     *
     * */
    public static function dataFilter($data,$type)
    {
        $retData=array();
        $default_val='';
        ...
        foreach($data as $k=>$v)
        {
            ...
            $val= $this->getVal($k,$default_val,$type);
            (isset($v[1]))||($v[1]='');
            (isset($v[2]))||($v[2]='');
            (isset($v[3]))||($v[3]='');
            if($val=='::')
            {
                $this->errorHandle('unlawful request (parameter is not complete)');
            }else if($val==$default_val)
            {
                $retData[$k]=$default_val;
            }else
            {
                $retData[$k]=$this->wordProcessing($val,$v[1],$v[2],$v[3]);
            }
        }
     ...
     * */
    public function getVal($k,$default_val,$type)
    {
        switch($type)
        {
            case 'post':
            ...
            default:
                $this->errorHandle('undefined request type : '.$type);
        }
    }
    /*
     * 对词进行相应的处理
     ...
     * */
    public function wordProcessing($val,$rule,$rule_detail,$memo='')
    {
        ...
        switch($rule)
        {
            ...
            case 'float':
                ...
                break;
            case 'regexp':
                $ret_val=$this->regularExpression($val,$rule_detail);
                break;
            case 'regexp_defined':
                if(isset(self::$preg[$rule_detail]))
                {
                    $ret_val=$this->regularExpression($val,self::$preg[$rule_detail]);
                    break;
                }
                else
                {
                    $this->errorHandle('regexp not found : '.$rule_detail);
                }
            case 'function':
            ...
            case 'xss':
                ...
                break;
            case 'unique':
                $flag=$this->ifUnique($val,$rule_detail[0],$rule_detail[1]);
            ...
            case 'not_unique':
                ...
                break;
            case 'length':
                $ret_val=$this->lengthCheck($val,$rule_detail);
                break;
            case 'multi':
                $ret_val=$this->multi($val,$rule_detail,$memo);
                break;
            case 'ignore':
                $ret_val=$val;
                break;
        ...
        if($ret_val===false)
        {
            $this->errorHandle($memo?$memo:'illegal data');
        }
        return $ret_val;
    }
    /*
     * 长度检测
     * */
    public function lengthCheck($val,$rule_detail)
    {
        $len=strlen($val);
     ...
     * 多个条件
     * */
    public function multi($val,$rule_detail,$memo='')
    {
        $ret_val='';
        foreach((Array)$rule_detail as $v)
        {
            (isset($v[1]))||($v[1]='');
            (isset($v[2]))||($v[2]=$memo);
            $ret_val=$this->wordProcessing($val,$v[0],$v[1],$v[2]);
        }
        return $ret_val;
    }
    /*
     * 判断是否唯一
     * */
    public function ifUnique($val,$table_name,$where)
    {
     ...
     * regular expression
     * */
    public function regularExpression($val,$rule_detail)
    {
